...
public class PetWalkSessionRepositoryImpl implements PetWalkSessionRepository {
    ...
    public PetWalkSessionRepositoryImpl(JpaPetWalkSessionRepository jpaRepository) {
        this.jpaRepository = jpaRepository;
    }

    @Override
    public PetWalkSession save(PetWalkSession session) {
        return jpaRepository.save(session);
    }

    @Override
    public Optional<PetWalkSession> findById(Long id) {
        return jpaRepository.findById(Objects.requireNonNull(id));
    }

    @Override
    public List<PetWalkSession> findByPetIdOrderByStartedAtDesc(Long petId) {
        return jpaRepository.findByPetIdOrderByStartedAtDesc(Objects.requireNonNull(petId));
    }

    @Override
    public List<PetWalkSession> findByStatusAndVisibilityOrderByStartedAtDesc(
            PetWalkSessionStatus status,
            PetVisibility visibility) {
        return jpaRepository.findByStatusAndVisibilityOrderByStartedAtDesc(status, visibility);
    }
}
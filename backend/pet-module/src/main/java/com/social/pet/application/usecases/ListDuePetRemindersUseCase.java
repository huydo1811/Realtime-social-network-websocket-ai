...
@Service
public class ListDuePetRemindersUseCase {
    private final PetHealthReminderRepository reminderRepository;

    public ListDuePetRemindersUseCase(PetHealthReminderRepository reminderRepository) {
        this.reminderRepository = reminderRepository;
    }

    @Transactional(readOnly = true)
    public List<PetHealthReminder> execute(Long actorId) {
        return reminderRepository.findDueByOwnerUserId(actorId, LocalDate.now(), PetReminderStatus.PENDING);
    }
}

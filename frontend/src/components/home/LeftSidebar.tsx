...
export default function LeftSidebar() {
  ...
  const { friendBadge, unreadChat, dismissFriendBadge, refreshFriendIncoming } = useNavBadgesContext();
  ...
  const isActive = (p: string) => {
    ...
    if (p === "/settings") return pathname === "/settings" || pathname.startsWith("/settings/");
  ...
  return (
    <aside className="hidden lg:flex flex-col w-64 xl:w-72 fixed inset-y-0 left-0 bg-white border-r border-slate-200 px-6 py-8 shadow-sm z-10">
      <div className="mb-8 flex items-center justify-between gap-2 pl-2 pr-1">
        <Link href="/" className="flex min-w-0 items-center gap-3">
          <div className="relative h-10 w-10 shrink-0 rounded-xl bg-gradient-to-br from-rose-500 to-rose-300 p-0.5 shadow-md">
            <Image src="/hype.png" alt="Hype logo" width={40} height={40} className="h-full w-full rounded-[10px] object-cover bg-white" />
          </div>
          <span className="truncate text-3xl font-extrabold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-rose-500 to-rose-400">
            Hype
          </span>
        </Link>
        <NotificationCenter
          panelPlacement="sidebar"
          buttonClassName="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-slate-600 transition hover:bg-rose-50 hover:text-rose-600"
        />
      </div>
      ...
      <nav className="flex flex-col gap-2 flex-1">
        ...
        <Link
          href="/friends"
          onClick={() => dismissFriendBadge()}
          className={`${base} ${isActive("/friends") ? active : inactive}`}
        >
          <svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
        ...
        <Link href="/pets" className={`${base} ${isActive("/pets") ? active : inactive}`}>
          <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
            <path d="M4.5 9C3.67 9 3 10.12 3 11.5S3.67 14 4.5 14 6 12.88 6 11.5 5.33 9 4.5 9Zm4.5-4c-1.1 0-2 1.34-2 3s.9 3 2 3 2-1.34 2-3-.9-3-2-3Zm6 0c-1.1 0-2 1.34-2 3s.9 3 2 3 2-1.34 2-3-.9-3-2-3Zm4.5 4c-.83 0-1.5 1.12-1.5 2.5s.67 2.5 1.5 2.5 1.5-1.12 1.5-2.5-.67-2.5-1.5-2.5ZM12 13c-4 0-7 2-7 4.5C5 19.43 6.57 21 8.5 21c1.34 0 2.5-.78 3.5-1.5 1 .72 2.16 1.5 3.5 1.5 1.93 0 3.5-1.57 3.5-3.5C19 15 16 13 12 13Z" />
          </svg>
          Thú cưng
        </Link>
        ...
        <Link href="/groups" className={`${base} ${isActive("/groups") ? active : inactive}`}>
          <svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
        ...
        <Link href="/settings" className={`${base} ${isActive("/settings") ? active : inactive}`}>
          <svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
